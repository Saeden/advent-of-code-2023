use std::{char, collections::HashMap, fmt::write};
...
fn pretty_print_gear(gear: &Point, map: &HashMap<Point, char>) {
...
fn day3_2() {
    ...
    get_numbers_map_and_gears(&mut numbers, &mut map, &mut gears);
...
fn day3_1() {
    ...
    get_numbers_and_map(&mut numbers, &mut map);
...
fn get_numbers_and_map(numbers: &mut Vec<Number>, map: &mut HashMap<Point, char>) {
    let mut number: i128 = 0;
    for (y, line) in line_iterator("input.txt").enumerate() {
        let line = line.expect("Failed to read line");
        //println!("({}): line {}", line, y);
...
fn get_number<'a>(
    ...
    if character.is_digit(10) && *x == line_len - 1 {
